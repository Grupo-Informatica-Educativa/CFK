...
def app():
    st.write("""# GreentTIC Análisis: Pretest piloto gamma (Diagrama de barras)""")
    ...
    if file:
        ...
        datos_2 = pd.read_excel(
            file, sheet_name='pregunta_{}'.format(numero_pregunta))
        ...
        if columna is not None:
            ...
            columna_aux = None
        tabla = pd.pivot_table(datos_2, values='Registro', index=list(
            set(arreglo_indices)), aggfunc='count').reset_index()
        tabla = tabla.sort_values(by=['Respuestas'], ascending=False)
        if str(numero_pregunta) == '3':
            if st.checkbox('Filtrar otras opciones'):
                tabla = tabla[tabla.Respuestas.isin(["No la conozco", "La evitaria", "Me interesa poco",
