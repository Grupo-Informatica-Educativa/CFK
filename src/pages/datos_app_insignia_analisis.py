...
def app():
	st.write("""# "GreentTIC Análisis: Datos App insignias" (Diagrama de barras)""")
	...
	if file:
		...
		arreglo_grafica_adicional = ['1. insignia(ruta)']

		indices_preguntas = [columnas.index(arreglo_grafica_adicional[0])]
		...
		if columna is not None:
			...
			columna_aux = None
		tabla = pd.pivot_table(datos_2, values='usuario_id', index=list(set(arreglo_indices)), aggfunc='count').reset_index()
