...
def grafica1_repertorios():
    ...
    fig = px.box(df, y="Valor", color="Tipo",title="Repertorios Específicos de Nivel Avanzado en Pre y Postest",color_discrete_sequence=color)
...
def grafica2_sexismonivelygenero():
    ...
    fig.for_each_annotation(lambda a: a.update(text=a.text.replace("Género=", "")))
    st.plotly_chart(fig, use_container_width=True)

def grafica21_seximobenevolbasico():
    aux_basico = pd.read_csv(path+"Grafica_21_Genero_Benevolente.csv")
    fig = px.box(aux_basico, y="Valor", color="Tipo de test",title="Sexismo benevolente en Pretest y Postest para nivel básico",color_discrete_sequence=color)
    fig.update_layout(title_x=0.5,height=600)
    st.plotly_chart(fig, use_container_width=True)

def grafica22_seximobenevolavanzado():
    aux_avanzado = pd.read_csv(path+"Grafica_22_Genero_Benevolente.csv")
    fig = px.box(aux_avanzado, y="Valor", color="Tipo de test",title="Sexismo benevolente en Pretest y Postest para nivel avanzado",color_discrete_sequence=color)
    fig.update_layout(title_x=0.5,height=600)
    st.plotly_chart(fig, use_container_width=True)
...
def grafica3_prejuicios():
    ...
    fig = px.bar(aux,y="Promedio del Puntaje",x="Genero",color="Etnia",barmode="group",title=title,color_discrete_sequence=color)
...
def grafica31_prejuicios():
    ...
    fig = px.bar(aux,y="Promedio del Puntaje",x="Genero",color="Estrato",barmode="group",title=title,color_discrete_sequence=color)
    fig.update_layout(title_x=0.5)
    st.plotly_chart(fig, use_container_width=True)
...
def grafica4_sociodemografica():
    ...
    fig = px.bar(aux,x="Respuesta",y="Porcentaje",color="Genero",barmode="group",title="Hombres y mujeres cabeza de hogar",color_discrete_sequence=color)
    fig.update_layout(title_x=0.5,height=600)
...
def grafica41_sociodemografica():
    ...
    fig = px.bar(aux,x="Grados",y="Porcentaje",color="Genero",barmode="group",facet_col="Materia",title=title,color_discrete_sequence=color)
    fig.update_layout(title_x=0.5,height=600)
    fig.update_xaxes(title='', visible=True, showticklabels=True)
    fig.for_each_annotation(lambda a: a.update(text=a.text.replace("Materia=", "")))
    fig.update_yaxes(tickformat="0.1%")
    st.plotly_chart(fig, use_container_width=True)
...
def grafica53_repertorios():
    ...
    fig = px.box(aux, y="Puntaje",title=title,labels=dict(Puntaje="Puntaje de Repertorios Generales"),color_discrete_sequence=color)
    fig.update_layout(title_x=0.5,height=600)
    st.plotly_chart(fig, use_container_width=True)
...
def app():
    ...
    grafica = st.selectbox("Seleccione una gráfica",q_list, format_func=lambda i: i['title'])
    grafica["func"]()
