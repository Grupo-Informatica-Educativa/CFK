...
def filtros_tabla(datos, col_preguntas, tipo_grafica, diccionario_preguntas={}):
    lista_filtros = []
    ...
    lista_preguntas_indices = [str("Pregunta ") + str(x + 1)
                               for x in range(longitud)]
    pregunta = st.selectbox("Seleccione la pregunta: ",
                            lista_preguntas_indices)
    ...
    for index, col in enumerate(cols):
        with col:
            if index == 0:
                lista_filtros.append(st.selectbox(
                    "Dividir por color", [" "] + lista_agrupadores))
            elif index == 1:
                lista_filtros.append(st.selectbox(
