...
def filtros_tabla(datos, col_preguntas, tipo_grafica, categoria=None, arreglo_preguntas=[], analisis=False):
	...
	if tipo_grafica == 'Cajas' or tipo_grafica == 'Dispersión':
		...
		if not analisis:
			...
			lista_filtros.append(st.selectbox("Seleccione el eje x", ["Pregunta"]))
	...
	for index, col in enumerate(cols):
		with col:
			if index == 0:
				if not analisis:
					lista_filtros.append(st.selectbox("Dividir por color", [" ", "Pregunta"] + lista_agrupadores_color))
				else:
					lista_filtros.append(st.selectbox("Dividir por color", ["Pregunta"]))
			elif index == 1:
				if not analisis:
